...
function parseStore(text, path) {
	...
	try {
		...
		throw new Error(`subscriptions auth store at ${path} is not valid JSON; fix or delete the store file`);
	}
	if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) throw new Error(`subscriptions auth store at ${path} must be a JSON object keyed by provider; fix or delete the store file`);
	const store = parsed;
