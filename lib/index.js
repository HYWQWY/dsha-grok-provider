...
/** Build id — must match the pushed chunk set. */
const BUILD = "0.1.3";
    ...
    if (typeof exp === "string") rel = exp;
      ...
      if (typeof dot === "string") rel = dot;
        ...
        if (rel && typeof rel === "object") {
          rel = rel.import || rel.default || rel.require;
        }
      }
    }
...
if (sha !== EXPECTED_SHA256) {
  throw new Error(
    `dsha-grok-provider ${BUILD}: chunk set mismatch (got ${sha.slice(0, 12)}…, expected ${EXPECTED_SHA256.slice(0, 12)}…). Reinstall the plugin from GitHub.`,
  );
}
const source = rewriteHostImports(sourceRaw);
const assembled = join(here, ".assembled.mjs");
writeFileSync(assembled, source, "utf8");
const mod = await import(`${pathToFileURL(assembled).href}?v=${BUILD}-${Date.now()}`);
