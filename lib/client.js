window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const I18N = {
			...
			manualPlaceholder: isZh ? "粘贴授权码，或地址栏里的 callback 完整链接" : "Paste auth code or full callback URL from the address bar",
			manualHint: isZh
				? "安卓/虚拟环境通常收不到 localhost 回调。在系统浏览器完成授权后，从地址栏复制整段链接（含 code=），或复制页面给出的授权码，粘贴后点提交。"
				: "On Android/proot, localhost callbacks usually fail. After authorizing in the system browser, copy the full address-bar URL (with code=) or the auth code shown, paste it here, then submit.",
			manualNeedLogin: isZh ? "请先点「登录」再粘贴授权码" : "Tap Sign In first, then paste the code",
		...
		function SubscriptionsSection(props) {
			...
			async function submitManual(provider) {
				...
				try {
					...
					if (!st.busy) {
						setErrors((e) => ({ ...e, [provider]: I18N.manualNeedLogin }));
						return;
					}
					...
					setErrors((e) => ({
						...e,
						[provider]: /no .* login attempt/i.test(msg) ? I18N.manualNeedLogin : msg
					}));
					await refresh();
				}
			}
			...
			return jsxs("div", { style: container, children: [
				...
				jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 10 }, children: PROVIDERS.map((p) => {
					...
					return jsxs("div", { key: p.id, style: cardBase, children: [
						...
						!isLogged ? jsxs("div", {
							style: { display: "flex", flexDirection: "column", gap: 8, paddingTop: 4, borderTop: "1px dashed var(--dsw-alias-border-l2, rgba(127,127,127,0.2))" },
							children: [
								jsx("div", { style: { ...metaBase, fontSize: 11, lineHeight: "16px" }, children: I18N.manualHint }),
								isBusy && authorizeUrl ? jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 4 }, children: [
									jsx("a", { href: authorizeUrl, target: "_blank", rel: "noreferrer", style: { fontSize: 12, color: "var(--dsw-alias-brand-primary, #1677ff)", wordBreak: "break-all" }, children: I18N.openAuthUrl }),
									jsx("button", {
										type: "button",
										style: { ...btnBase, alignSelf: "flex-start", height: 24, padding: "0 8px", fontSize: 11 },
										onClick: () => copyText(`auth_${p.id}`, authorizeUrl),
										children: copied[`auth_${p.id}`] ? I18N.copied : (isZh ? "复制授权链接" : "Copy auth URL")
									})
								] }) : null,
								jsxs("div", { style: { display: "flex", gap: 8, alignItems: "stretch" }, children: [
									jsx("input", {
										style: { ...inputBase, minHeight: 36, fontSize: 16, WebkitUserSelect: "text", userSelect: "text" },
										...
										spellCheck: false,
										inputMode: "text",
										onChange: (event) => setManual((m) => ({ ...m, [p.id]: event.target.value }))
									}),
									jsx("button", {
										type: "button",
										style: { ...btnBase, minHeight: 36, background: "var(--dsw-alias-brand-primary, #1677ff)", color: "#fff", border: "none", flexShrink: 0 },
										onClick: () => { void submitManual(p.id); },
